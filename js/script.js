...
//     document.getElementById('alt-GridTeam').innerHTML = "";
//     for (let i = 0; i < data.results.length; i++) {
//         document.getElementById('alt-GridTeam').innerHTML += `
//     <div class="sectionOne participant">
//             <img crossOrigin="anonymous" src="${data.results[i].image}" alt="">
//             <div class="position">
...
document.addEventListener("DOMContentLoaded", async function () {
    const reviewsContainer = document.querySelector(".alt-reviews-container");

    async function fetchReviews() {
        try {
            const response = await fetch("https://api.msp-alazhar.tech/reviews?page=1&limit=5");
            const data = await response.json();
            return data.results || [];
        } catch (error) {
    ...
    function createReviewCard(review) {
        ...
        card.classList.add("alt-review-card");

        card.innerHTML = `
            <p class="alt-review-title">${review.title || "No Title"}</p>
            <p class="alt-review-text">"${review.review}"</p>
            <div class="alt-review-divider"></div>
            <div class="alt-review-footer">
                <img crossOrigin="anonymous" src="${review.photo || 'default-user.png'}" alt="User">
                <div class="alt-review-stars">★★★★☆</div>
            </div>
        `;
        return card;
    }
    ...
    async function displayReviews() {
        ...
        if (reviews.length === 0) {
            reviewsContainer.innerHTML = "<p class='alt-no-reviews'>No reviews available.</p>";
            return;
        }
        ...
        let currentIndex = 0;
        setInterval(() => {
            const cards = document.querySelectorAll(".alt-review-card");
            if (cards.length > 1) {
                cards[currentIndex].classList.add("fade-out");

                setTimeout(() => {
                    reviewsContainer.removeChild(cards[currentIndex]);

                    const newCard = createReviewCard(reviews[currentIndex]);
...
//  end of display review cards 
// start of fetch blogs
async function fetchBlogs() {
    try {
        const response = await fetch('https://api.msp-alazhar.tech/blogsClient/get?page=1&limit=3');
        const data = await response.json();
